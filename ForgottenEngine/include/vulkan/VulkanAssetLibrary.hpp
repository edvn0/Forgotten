...
namespace ForgottenEngine {
	...
	public:
		...
		const std::shared_ptr<RenderMaterial>& create_material(VkPipeline pipeline, VkPipelineLayout layout, const std::string& name);

		void add_mesh(const std::string& name, std::shared_ptr<Mesh>&& new_mesh) { meshes[name] = std::move(new_mesh); }
		...
		void add_texture(const std::string& name, std::shared_ptr<Image>&& texture) { textures[name] = std::move(texture); }

...
namespace ForgottenEngine::UI {
		...
		const Reference<Image2D>& image, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
	{
		if (RendererAPI::current() == RendererAPIType::Vulkan) {
			...
			const auto textureID
				= ImGui_ImplVulkan_AddTexture(imageInfo.sampler, imageInfo.image_view, vulkanImage->get_descriptor_info().imageLayout);
			ImGui::Image((ImU64)textureID, size, uv0, uv1, tint_col, border_col);
		}
	}

	void image(const Reference<Image2D>& image, uint32_t imageLayer, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col,
	...
	bool image_button(const char* stringID, const Reference<Image2D>& image, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
		int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
	{
		if (RendererAPI::current() == RendererAPIType::Vulkan) {
			...
			return ImGui::ImageButtonEx(
				id, (ImU64)textureID, size, uv0, uv1, ImVec2 { (float)frame_padding, (float)frame_padding }, bg_col, tint_col);
		}
	...
	bool image_button(const char* stringID, const Reference<Texture2D>& texture, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1,
		int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
	{

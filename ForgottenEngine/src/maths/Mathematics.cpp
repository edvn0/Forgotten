...
namespace ForgottenEngine::Maths {
	...
	glm::vec3 scale_vector(const glm::vec3& v, float desiredLength) { return v * desiredLength / length(v); }
	...
	bool decompose_transform(const glm::mat4& transform, glm::vec3& translation, glm::quat& rotation, glm::vec3& scale)
	{
		...
		Row[0] = scale_vector(Row[0], static_cast<T>(1));
		...
		Row[1] = scale_vector(Row[1], static_cast<T>(1));
		// Skew.z /= Scale.y;
		...
		Row[2] = scale_vector(Row[2], static_cast<T>(1));
		// Skew.y /= Scale.z;
		// Skew.x /= Scale.z;

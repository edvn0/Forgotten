...
#include "implementations/Windows/WindowsWindow.hpp"
...
Window* Window::create(const ApplicationProperties& props) { return new WindowsWindow(props); };

WindowsWindow::WindowsWindow(ApplicationProperties props)
...
void WindowsWindow::resize_window(float w, float h) const { glfwSetWindowSize(glfw_window, (int)w, (int)h); }

void WindowsWindow::resize_framebuffer(int w, int h) const
{
	(void)w;
	(void)h;
}
...
void WindowsWindow::process_events() { glfwPollEvents(); }

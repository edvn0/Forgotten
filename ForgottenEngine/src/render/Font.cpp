...
static Reference<Texture2D> CreateAndCacheAtlas(const std::string& fontName, float fontSize, const std::vector<GlyphGeometry>& glyphs, const FontGeometry& fontGeometry, const Configuration& config)
{
	...
	: m_FilePath(filepath), m_MSDFData(new MSDFData())
{
	...
	{
		CORE_WARN("Missing {0} {1}", (int)charset.size() - glyphsLoaded, fontInput.glyphIdentifierType == GlyphIdentifierType::UNICODE_CODEPOINT ? "codepoints" : "glyphs");
	}
	...
	if (int remaining = atlasPacker.pack(m_MSDFData->Glyphs.data(), m_MSDFData->Glyphs.size()))
	{
		...
		{
			CORE_ERROR("Error: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
			CORE_ASSERT(false, "");
		}
	}
...
void Font::init()
{
	s_DefaultFont = Reference<Font>::create("Resources/Fonts/opensans/OpenSans-Regular.ttf");
}
...
Reference<Font> Font::get_default_font()
{
